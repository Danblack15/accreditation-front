	...
	data() {
		return {
			...
			listText: [
				...
				'Вы можете выбрать только ограниченное количество категорий из предложенного списка, поэтому постарайтесь подойти к этому вопросу серьёзнее, именно от категорий будут зависеть предложенные ГОСТы',
				'Соответствую выбранным категориям система предложит вам ГОСТы, с нимим можно будет ознакомиться, скачав их.',
				'Каждому ГОСТу будет присвоен выпадающий список, в котором содержатся выдержки из данного ГОСТа, а именно рекомендации по поводу аппаратуры для проверки и методов проверки, которые потенциально приведут к одобрению сертификации. данные можно будет выгрузить в Exel'
			],
			active: 1
		}
	},
